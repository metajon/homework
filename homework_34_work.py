...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
		...
		if a > 0.0:
			R = x/b
			cx = x - sin(theta)*R
			cy = myrobot.y + cos(theta)*R
        	myrobot.x = cx + sin(theta+b)*R
        	myrobot.y = cy - cos(theta+b)*R
        	myrobot.orientation = (theta+b)%2*pi
        else:
        	myrobot.x = myrobot.x + x * cos(theta)
        	myrobot.y = myrobot.y - x * sin(theta)
        	myrobot.orientation = (theta+b)%2*pi
